function [grads_theta,grads_tau,loss] = grad_ddesimu(tt,t_simu,dltheta,dltau,dlhist_vec,hist_t,targets,lm,st,w)
loss = 0;
...
%         newloss = 1/sqrt(st(mod(i-1,length(st))+1))*newloss+0.1*mean(dlx(:,2:end)>1,'all')+0.1*mean(dlx(:,2:end)<0,'all');
    end
    loss = loss + newloss;
end
